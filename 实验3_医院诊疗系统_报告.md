...
- 建表脚本：`sql/schema.sql`
- 程序侧自动建表/默认用户：`db/dbmanager.cpp`（`DbManager::ensureSchema()` / `DbManager::seedDefaultUser()`）
...
- 登录页 UI：`ui/loginpage.cpp`（`QFormLayout + QLineEdit + QPushButton` 代码化布局）
- 主页 UI：`ui/homepage.cpp`（`QGridLayout` 三个入口按钮）

关键实现点：

- 采用纯代码生成 UI（无 `.ui` 依赖），便于定制工具栏/页面布局
- 登录页包含“注册/登录”，主页包含“科室/医生/患者”入口
...
- 工具栏：`mainwindow.cpp`（代码添加 `QToolBar` 与 `QAction`：左侧返回；中间页面标题；右侧菜单下拉：日志/退出登录）

主要代码（摘要）：

- 页面栈创建：`MainWindow::buildUi()`（创建各 Page 并 `m_stack->addWidget(...)`）
- 页面切换：`MainWindow::setPage(Page)`（统一跳转入口）
- 工具栏显隐：`MainWindow::updateChrome()`（根据登录/页面状态控制按钮）
...
void MainWindow::setPage(Page p) {
    m_page = p;
    switch (p) {
    case Page::Login: m_stack->setCurrentWidget(m_login); break;
    case Page::Home: m_stack->setCurrentWidget(m_home); break;
    case Page::Patients: m_stack->setCurrentWidget(m_patients); break;
    default: break;
    }
    updateChrome();
}
...
- 打开数据库：`DbManager::open()`（SQLite + `PRAGMA foreign_keys=ON`）
- 通用执行：`DbManager::exec(sql,args)`（`prepare + bind + exec`）
- 通用查询展示：`DbManager::createQueryModel(sql,args)`（返回 `QSqlQueryModel*`，用于 History 页面）
...
- 登录校验：`LoginPage::onLoginClicked()`（`SELECT ... FROM User WHERE USERNAME=? AND PASSWORD=?`）
- 登录成功信号：`LoginPage::loginSucceeded(const UserInfo&)`
- 主窗口接收信号并跳转：`mainwindow.cpp` 连接 `loginSucceeded` → `setPage(Page::Home)`
- 操作记录：`HistoryLogger::logEvent(userId, event)` 写入 `History` 表

关键代码（节选）：

```cpp
QSqlQuery q(DbManager::instance().database());
q.prepare("SELECT ID,FULLNAME FROM User WHERE USERNAME=? AND PASSWORD=? LIMIT 1;");
...
- 列表数据：`PatientModel`（`setTable("Patient") + select()`）
- 查询过滤：`PatientModel::setKeywordFilter()`（身份证/姓名/手机号 `LIKE` 过滤，带 `ESCAPE`）
- 删除：`PatientPage::onDelete()`（确认对话框 + `DELETE FROM Patient WHERE ID=?`）
- 自定义编辑器：`PatientDelegate::createEditor()`（性别/日期/数值列的编辑控件）

关键代码（节选：查询过滤）：

```cpp
setFilter("(ID_CARD LIKE '%1' ESCAPE '\\' OR NAME LIKE '%1' ESCAPE '\\' OR MOBILEPHONE LIKE '%1' ESCAPE '\\')")
    .arg(like);
select();
```
...
- 弹窗编辑：`PatientEditDialog`（表单控件：姓名/身份证/性别/出生日期/身高体重/手机号/年龄）
- 添加：`PatientPage::onAdd()` → `insertPatient(...)`（`INSERT INTO Patient(...) VALUES(...)`）
- 修改：`PatientPage::onEdit()` → `updatePatient(...)`（`UPDATE Patient SET ... WHERE ID=?`）
- 自动生成主键：`QUuid::createUuid()`

关键代码（节选：新增）：

```cpp
return DbManager::instance().exec(
  "INSERT INTO Patient(ID,ID_CARD,NAME,SEX,DOB,HEIGHT,WEIGHT,MOBILEPHONE,AGE,CREATEDTIMESTAMP) VALUES(?,?,?,?,?,?,?,?,?,?);",
  {p.id, p.idCard, p.name, p.sex, p.dob.toString(Qt::ISODate), p.height, p.weight, p.mobilePhone, p.age, created},
  error);
```
